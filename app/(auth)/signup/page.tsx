...
export default function SignupPage() {
  ...
  // Show email confirmation dialog on success
  if (state?.success) {
    return (
      <div className="bg-surface-900/80 backdrop-blur-xl border border-surface-700/50 rounded-2xl p-8 shadow-2xl text-center">
        <div className="w-16 h-16 mx-auto mb-5 rounded-full bg-primary-500/10 border border-primary-500/20 flex items-center justify-center">
          <svg
        ...
        <p className="text-primary-400 font-medium text-sm mb-5">
          {(state.data?.email as string) || "your email"}
        </p>
        ...
        <Link
          href="/login"
          className="inline-flex items-center gap-2 px-6 py-2.5 rounded-lg bg-primary-600 hover:bg-primary-500 text-white font-medium text-sm transition-all shadow-lg shadow-primary-600/20 cursor-pointer"
        >
          Go to Sign in
        </Link>
      </div>
